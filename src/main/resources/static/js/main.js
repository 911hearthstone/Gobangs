...
function start(){
    $.ajax({
        ...
        timeout:600000,
            ...
            flag=result;
            console.log("flag="+sendflag);
        }
    });
}
//控制翻页的状态,0(AI vs AI),1(Player vs AI 白子先行)，2（Player vs Player）
var count=0;
//默认是白子，1为黑子，发送Player vs AI 黑子先行给下位机
var colour=0;
// 上一页
function prev() {
...
function initialLi() {
    var black=$('.gamer').children("ul:first").children("li:first");
    var white=$('.gamer').children("ul:first").children("li:last");
    black.removeClass('active');//黑色去掉高亮
    white.addClass('active');//白色添加高亮
}


    // 选择棋方
    $('.gamer').delegate('li', 'click', function (event) {
        if ($(this).hasClass('active')) {
            return false;
        }//已经选中则不变
        $(this).siblings().removeClass('active');//全部去掉class属性
        $(this).addClass('active');//给当前li标签添加class属性
        var v = $(this).attr('value');
        if (v == 0) {
            colour = 1;
            $.ajax({
                type: "POST",
                url: "/colour?sendflag=" + sendflag,
                data: {},
                cache: "false",
                timeout: 600000,
                success: function (result) {
...
// 0 代表黑色 1 代表白色
function getColor() {
    let color = $('.gamer li[class=active]').attr('value');
    return color;
}
